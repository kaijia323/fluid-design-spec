...
- 页签 3–5 个，图标 + 文字，当前项用主色 `--color-action-default`（组件只引用语义层令牌，明暗自动切到品牌提亮色）。
...
| 向上滚动 | 导航自动浮现 |
| 隐藏/浮现时长 | 200ms |
| 当前页指示器 | 使用凝光小点（跟随品牌主色的小光点），不是粗下划线 |
...
- 光斑随指针 / 触摸位置流动：`radial-gradient` 跟随指针坐标，用 CSS 变量传坐标（※）；光斑颜色用品牌主色的低透明度版本（`color-mix(in srgb, var(--brand-accent) 18%, transparent)`），**不要写死品牌 hex**。这是规范里**唯一允许直接读品牌层**的地方（语义层没有"品牌色低透明度"这类令牌，见 `Design.md` 2.1）；`color-mix()` 需要 Chrome 111+ / Safari 16.2+；要兼容老 WebView，就在 `tokens/brand.css` 里补一个低透明度的品牌色令牌（**当前不存在，需按需新增**；品牌 hex 仍只允许出现在该文件），再用 `@supports not (color: color-mix(in srgb, red, blue))` 切过去——**不要**在组件里直接写死 `rgba()`。
...
- 四态齐全：默认 / 悬停 / 聚焦（品牌主色 2px 描边 + 标签上浮）/ 错误或禁用。
...
- [ ] 品牌色是否只来自 `tokens/brand.css`？按下态的主色块文字是否用了 `--color-on-accent`？
